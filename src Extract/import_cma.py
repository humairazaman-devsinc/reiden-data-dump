...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('cma_import.log'),
...
class CMAImporter:
    ...
    def _fetch_cma_data(self, property_id: int, alias: str, currency: str, measurement: str, 
                       property_type: str, activity_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Fetching CMA data for property {property_id} with params {params}")
            ...
            if response and response.get('data'):
                ...
                logger.warning(f"No data returned for property {property_id} with params {params}")
                return None
    ...
    def _api_worker(self, task: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Fetch data
            raw_data = self._fetch_cma_data(property_id, alias, currency, measurement, property_type, activity_type)
    ...
    def process_cma(self, country_code: str = "AE", max_properties: int = 100, 
                   max_combinations: int = 16, offset: int = 0, dry_run: bool = False):
        ...
        try:
            ...
            logger.info(f"Generated {len(api_tasks)} API tasks for {len(properties)} properties")
            logger.info(f"Average {len(api_tasks) / len(properties):.1f} combinations per property")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Import CMA data from Reidin API with dynamic schema")
