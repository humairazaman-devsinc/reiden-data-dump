...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("cma2_rents_import.log")],
)
...
class CMA2RentsImporter:
    def __init__(
        ...
        self.property_subtypes = ["Medical Office", "Office", "Whole Building"]
        ...
        while True:
            try:
                ...
                if (
                    ...
                    or (time.time() - batch_start_time) > 5.0
                ):
                    self._flush_batch_to_db(batch)
                    batch = []
                    batch_start_time = time.time()
    ...
    def _flush_batch_to_db(self, batch: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully inserted {len(batch)} records in {elapsed:.2f}s ({rate:.1f} rec/s) (Total: {self.total_records_inserted} records, {self.total_batches_flushed} batches)"
            )
    ...
    def generate_parameter_combinations(
        self, max_combinations_per_property
    ) -> List[Dict[str, Any]]:
    ...
    def _process_cma2_rents_response(
        self,
        ...
        try:
            ...
            if isinstance(response_data, dict):
                ...
                if (
                    response_data.get("info", {}).get("avg_price") == "Result is empty!"
                    or response_data.get("results") == []
                    or not response_data.get("results")
                ):
                    logger.info(
                        f"Skipping property {property_id} - API returned empty results"
                    )
            ...
            if isinstance(response_data, dict):
                data_items = (
                    response_data.get("results") or response_data.get("data") or []
                )
            ...
            if not data_items:
                logger.info(
                    f"Skipping property {property_id} - No data items in response"
                )
                return processed
            ...
            avg_rent_per_sqm = (
                sum(prices_per_sqm) / len(prices_per_sqm) if prices_per_sqm else 0
            )
            ...
            processed_record = {
                ...
                "raw_data": {
                    "transactions": valid_transactions[0] if valid_transactions else {}
                },  # Store first transaction as single object
            }
    ...
    def process_single_property(
        self, property_data: Dict[str, Any], max_combinations_per_property: int = 10
    ) -> int:
        ...
        for combination in self.generate_parameter_combinations(
            ...
            alias = combination.get("alias")
            currency = combination.get("currency")
            measurement = combination.get("measurement")
            property_type = combination.get("property_type")
            property_subtype = combination.get("property_subtype")
            ...
            try:
                ...
                if response_data:
                    ...
                    processed_records = self._process_cma2_rents_response(
                        response_data,
                        property_id=property_id,
                        alias=alias,
                        currency=currency,
                ...
                logger.warning(
                    f"Failed to process property {property_id} with combination {alias}/{currency}/{measurement}/{property_type}/{property_subtype}: {e}"
                )
                continue
    ...
    def process_cma2_rents(
        ...
        remaining_properties = [
            p for p in properties if p["id"] not in processed_property_ids
        ]

        if not remaining_properties:
            logger.info(
                "✅ All properties have already been processed with current combinations"
            )
            return

        logger.info(
            f"🔄 Resume mode: Skipped {len(properties) - len(remaining_properties)} already processed tasks, {len(remaining_properties)} remaining"
        )
        ...
        try:
            ...
            for i in range(0, len(remaining_properties), batch_size):
                ...
                logger.info(
                    f"📦 Processing batch {i//batch_size + 1}/{(len(remaining_properties) + batch_size - 1)//batch_size} ({len(batch)} properties)"
                )
                ...
                logger.info(
                    f"📊 Progress: {processed_so_far}/{len(remaining_properties)} properties processed"
                )
            ...
            logger.info(
                f"Average records per batch: {self.total_records_inserted / max(1, self.total_batches_flushed):.1f}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CMA2-Rents ETL with modern architecture"
    )
